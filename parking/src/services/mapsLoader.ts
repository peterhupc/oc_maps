...
export function loadMaps(): Promise<typeof google.maps> {
  ...
  if (!apiKey) {
    return Promise.reject(
      new Error('缺少 VITE_GMAPS_KEY：請在 parking/.env.local 設定 Google Maps API Key')
    )
  }
  ...
  mapsPromise = new Promise((resolve, reject) => {
    ...
    w.__parkingMapsReady = () => resolve(google.maps)

    const script = document.createElement('script')
    script.src =
      'https://maps.googleapis.com/maps/api/js' +
      '?key=' + encodeURIComponent(apiKey) +
      '&libraries=places&language=zh-TW&region=TW&callback=__parkingMapsReady'
    script.async = true
    script.defer = true
