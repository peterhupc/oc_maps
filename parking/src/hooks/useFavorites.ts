...
const KEY = 'parking_favorites'
const FAV_COLLECTION = (uid: string) => `users/${uid}/parking_favorites`

export interface FavoriteItem extends ParkingPlace {
  savedAt: number
}
...
export function useFavorites() {
  ...
  const syncCloud = useCallback(async (uid: string, items: FavoriteItem[]) => {
    ...
    if (!db) return
    const favColl = collection(db, FAV_COLLECTION(uid))
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void (async () => {
      const snap = await getDocs(collection(db, FAV_COLLECTION(user.uid)))
      if (cancelled) return
  ...
  const toggle = useCallback(
    (place: ParkingPlace) => {
      setFavorites((prev) => {
        ...
        const db = getFirebaseDb()
        if (user && db) {
          const favRef = doc(db, `${FAV_COLLECTION(user.uid)}/${place.place_id}`)
